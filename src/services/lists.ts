...
export const WorkspaceMemberAccess = Schema.Struct({
	...
	level: Schema.Literals(["full-access", "read-and-write", "read-only"]),
})
...
export const List = Schema.Struct({
	...
	workspace_access: Schema.NullOr(
		Schema.Literals(["full-access", "read-and-write", "read-only"]),
	),
...
const makeAttioLists = Effect.gen(function* () {
	...
	return {
		 ...
		 */
		list: Effect.fn("AttioLists.list")(function* () {
		 ...
		 */
		create: Effect.fn("AttioLists.create")(function* (
			list: (typeof ListInput)["Encoded"],
		) {
			const data = yield* Schema.encodeUnknownEffect(ListInput)(list)
		 ...
		 * Required scopes: `list_configuration:read`
		 */
		get: Effect.fn("AttioLists.get")(function* (list: string) {
		 ...
		 * Required scopes: `list_configuration:read-write`
		 */
		update: Effect.fn("AttioLists.update")(function* (
			list: string,
			update: (typeof ListUpdate)["Encoded"],
		) {
			const data = yield* Schema.encodeUnknownEffect(ListUpdate)(update)
...
export class AttioLists extends Context.Service<
	AttioLists,
	Effect.Success<typeof makeAttioLists>
>()("effect-attio/services/AttioLists") {
